...
Three return paths from `I` to `S`: slow natural clearance, treatment of screened-positive indices, and treatment of notified partners (PR or EPT). Reinfection is allowed throughout, which is the whole point of the SIS framing: the post-PN equilibrium prevalence is what we measure, not the time to elimination.
...
1. Three flags on `control.net()`:
   ...
   ```
   `truncate.el.cuml` is **destructive**: edges older than the window are dropped from storage and cannot be recovered. Pass the maximum lookback across all your scenarios so no scenario starves.
...
3. The unique-vs-positional ID round-trip. `get_partners()` returns **unique IDs** in the `partner` column, because past partners may have already departed the population and no longer have a positional ID. Per-node attributes (`status`, `active`, etc.) are indexed by **positional ID**. Convert before indexing:
   ...
   ```
   `only.active.nodes = TRUE` guarantees the returned partners are still active, so the conversion will succeed, but the conversion is still required. Defensively drop any `NA`s that slip through (e.g. a partner who departs between the cumulative-edgelist update and the conversion call).

This is the single most common stumbling block. Treat the unique-vs-positional distinction as a hard rule of the cumulative-edgelist API: **anything coming back from `get_partners()` is in unique-ID space, convert before doing anything else**.
...
### Partner Notification Module (the headline)

The `partner_services` module is short on purpose. It identifies fresh-positive indices, asks the cumulative edgelist for their recent partners, converts unique IDs to positional IDs, applies a Bernoulli trace, and stamps the reached partners with `pn.notified = at`. The downstream `treat` module reads that stamp to decide what to do with each notified partner under the active PN arm.
...
3. **The gotcha.** `part_df$partner` is in unique-ID space. Every per-node vector in `dat` is in positional-ID space. Convert before indexing. Without this round-trip the next line would silently corrupt the state of the wrong nodes.
4. A partner could also be a fresh index this step. They are handled by the index-treatment path in `treat()`; double-counting them as notified partners would inflate the cascade stats.
5. The Bernoulli trace is the "successfully notified" rate. In reality this is the product of contact-information availability, contact-tracer effort, and partner cooperation. Here it is one tunable knob.
...
A modest sexual contact network with some concurrency. Each timestep is one week.
...
The cumulative-edgelist machinery is enabled here. `truncate.el.cuml` is set to the maximum lookback across scenarios so no scenario starves.
...
sim_pr <- netsim(est,
                 make_param(pn.arm = "PR",
                            pn.trace.prob = 0.5, pn.lookback = 60),
                 init, control)
...
sim_ept_long <- netsim(est,
                       make_param(pn.arm = "EPT",
                                  pn.trace.prob = 0.5, pn.lookback = 120),
                       init, control)

# Scenario 5: EPT, high trace (0.8) + longer lookback (120)
sim_ept_max <- netsim(est,
                      make_param(pn.arm = "EPT",
                                 pn.trace.prob = 0.8, pn.lookback = 120),
                      init, control)
```

The teaching arc here is:
...
- Scenario 5 stacks the levers: high trace plus long lookback. The improvement over Scenario 4 should be visible but moderate.
...
For each scenario we compute the subsequent infections per index across the post-PN window. The metric is a population-level proxy for "how many reinfections does the average diagnosed individual contribute to over the post-PN era". Lower means a tighter intervention loop.

```{r}
#| label: fig-reinfection
#| fig-cap: "Reinfection pressure (subsequent infections per index in the post-PN window) by scenario. Wider boxes mean more reinfection in the population per detected index."
...
| Parameter | Description | Default |
|-----------|-------------|---------|
| `screen.rate` | Weekly probability of screening for an infected | 0.025 (~40 wk between screens) |
