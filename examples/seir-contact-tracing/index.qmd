...
2. Required. Without this switch no partnership history is kept and `get_partners()` returns nothing.
...
4. The explicit order discussed above. Every module that should run must be listed; one left out is skipped without a warning.
...
The no-intervention and isolation-only epidemics are large and their final sizes vary little between simulations. The tracing scenarios are closer to the epidemic threshold, so their coefficients of variation are larger and their intervals wider, which is the honest state of any comparison between interventions that hold an epidemic near criticality. With ten simulations the intervals separate the coverage contrast cleanly and leave the speed contrast at the limit of resolution; more simulations narrow them in proportion to the square root of their number, and the run time scales linearly.
