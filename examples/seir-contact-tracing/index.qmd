...
infect <- function(dat, at) {
  ...
  del <- discord_edgelist(dat, at, network = 1,             # <1>
                          infstat = c("ip", "is"))
...
```
1. `discord_edgelist()` accepts a vector of infectious states through its `infstat` argument (it filters with `%in%`), so the Ip + Is split is handled in a single call.
...
```{r}
#| label: fig-incidence
#| fig-cap: "Daily new infections by scenario (7-day centered moving average across simulations, raw means shown faintly). Fast tracing flattens the peak; slow tracing shifts it without flattening."
par(mfrow = c(1, 1), mar = c(3.5, 4, 2.5, 1), mgp = c(2.4, 1, 0))
