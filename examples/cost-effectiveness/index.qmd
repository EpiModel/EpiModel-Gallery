...
The same CEA calculation performed externally using compartment counts (`s.num`, `i.num`) extracted from simulation output. This demonstrates that a custom module is not strictly necessary --- useful when adding CEA to an existing model without modifying modules.

All CEA parameters and the run horizon are read directly from the `sim` object so the two approaches cannot silently diverge if the simulation parameters change. Results should match Approach 1 exactly.

```{r}
#| label: cea-approach2
calc_outcomes <- function(sim, intervention) {

  # CEA parameters: pulled from the simulation's stored param list so they
  # are identical to what the costeffect module used.
  p <- sim$param
