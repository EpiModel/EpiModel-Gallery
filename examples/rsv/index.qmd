...
```{r}
#| label: fig-age-strat
#| fig-cap: "Cumulative attack rate by age group across the five scenarios. The Y axis is cumulative infections as a fraction of the age-group population."
#| eval: false
# See model.R for the full plotting code. The shape that matters:
# - Infant panel: infant_proph (purple) and both (dark blue) clearly below baseline.
# - Elderly panel: elderly_vax (green) and both clearly below baseline.
# - School/Adult: NPI (red) is the dominant intervention.
```

### Hospitalization Burden

Hospitalizations are derived post-hoc by multiplying age-stratified cumulative infections by published age-specific per-infection hospitalization risks:
...
```{r}
#| label: fig-hosp
#| fig-cap: "Stacked hospitalizations by age and scenario. Elderly hospitalizations dominate the baseline; elderly_vax and 'both' cut them substantially, NPI cuts moderately across all ages, infant_proph only affects the (small) infant slice."
#| eval: false
hosp_mat <- sapply(hosp_by_scn, function(h) h)
barplot(hosp_mat, names.arg = names(sims), las = 2,
        col = c("#3498db","#f39c12","#e74c3c","#27ae60","#8e44ad"),
...
| Scenario | Doses | Hosp averted | Per dose |
|---|---|---|---|
| Elderly vax (70% coverage, 65+) | ~530 | ~11.5 | **0.022** |
| Infant prophylaxis (80% coverage, 0-1) | ~55 | small absolute | ~0.015 (counting only infant hosps) |
| Both | ~590 | ~12.5 | 0.021 |
| NPI | 0 (no per-dose metric) | ~9 | -- |

The per-dose comparison reveals a real RSV policy trade-off: infant prophylaxis is comparable per-dose in its targeted age group, but the absolute number of doses delivered is small (infants are 1% of the population), so total averted hospitalizations are modest. Older-adult vaccination achieves higher absolute impact because there are more elderly to vaccinate AND each elderly infection carries a high (3%) hospitalization risk.
...
- **Cross-layer dependency.** The current implementation treats the two layers as independent. See [Multinets](../multinets/) for a `dat.updates` callback that links degree across layers.
