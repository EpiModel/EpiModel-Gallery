...
progress <- function(dat, at) {
  ...
  dat <- set_epi(dat, "AIDS.ART.flow",at, ifelse(length(idsEligAIDSART) > 0 &
                                        length(vecAIDSART) > 0,
                                      length(idsAIDSART), 0))
